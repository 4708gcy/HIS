...
1. 在 `Patient.h` 中添加并实现了患者个人信息管理功能，包括查看个人信息和修改个人信息两个部分，完善了患者的个人信息管理功能；并将昨天的患者住院信息管理功能进行了优化
2. 在 `Login.h` 中添加并实现了患者登录函数，实现了患者的登录功能
3. 在 `main.cpp` 中完善了患者的模块，初步搭建了患者的主功能框架
4. 在 `main.cpp` 中初步完成了用户注册模块(管理员注册、医生注册、护士注册、药剂师注册、患者注册)
5. 在 `UI.h` 中添加并实现了患者个人信息管理菜单，个人信息查看菜单和个人信息修改菜单
6. 拉取了小组成员的 `Nurse.h`, `Nurse.cpp`, `Pharmacist.h`, `Pharmacist.cpp`，并进行了功能的组合封装，完善了护士和药剂师的个人信息管理功能、医疗信息管理功能以及登录功能；并且在 `main.cpp` 中初步搭建了护士和药剂师的主功能框架