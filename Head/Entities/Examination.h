...
struct VitalSigns
{
    double temperatureC = 0.0;       // 体温（摄氏度）——体温测量
    double systolicBP = 0.0;         // 收缩压（mmHg）——血压测量
    double diastolicBP = 0.0;        // 舒张压（mmHg）——血压测量
    double heartRate = 0.0;          // 心率（次/分钟）——心率测量/脉搏
    double respiratoryRate = 0.0;    // 呼吸频率（次/分钟）——呼吸频率测量
    double spo2 = 0.0;               // 血氧饱和度（%）——脉搏血氧测量
    double height = 0.0;             // 身高（cm）——身高测量
    double weight = 0.0;             // 体重（kg）——体重测量
    double bmi = 0.0;                // 体质指数——BMI计算
    double painScore = 0.0;          // 疼痛评分（0-10）——疼痛评估
    double waistCircumference = 0.0; // 腰围（cm）——腰围测量
    double bloodSugar = 0.0;         // 血糖（mmol/L）——血糖测量
    double bodyFat = 0.0;            // 体脂率（%）——体脂测量
    double uricAcid = 0.0;           // 尿酸（μmol/L）——尿酸测定
    double cholesterol = 0.0;        // 总胆固醇（mmol/L）——血脂测定
    // 可根据实际需求继续扩展
};
