...
struct MonthlyDeptStats {
  ...
  long long totalHospitalizationCost = 0; // 单位：分
  int totalBedDays = 0;
};
...
struct DemandPrediction {
  ...
  double holtWintersForecast = 0.0; // Holt-Winters 双指数平滑预测
};
...
struct DeptMedicineStats {
  ...
  int totalStock = 0;
  long long totalPurchaseValue = 0; // 单位：分
  long long totalSaleValue = 0;     // 单位：分
...
struct DataAnalysisReport {
  ...
  std::vector<std::string> anomalies; // Z-score 异常检测结果
};
...
std::vector<MonthlyDeptStats> computeMonthlyStats(Hospitalization *hosHead,
                                                  Registration *regHead);
std::vector<DemandPrediction>
predictDemand(const std::vector<MonthlyDeptStats> &monthlyStats);
std::vector<DeptBedAnalysis>
analyzeBedAllocation(bedInfo *bedHead,
                     const std::vector<DemandPrediction> &predictions);
std::vector<BedReallocationSuggestion>
suggestBedReallocation(const std::vector<DeptBedAnalysis> &analyses);
std::vector<DeptMedicineStats> analyzeMedicineStats(Medicine *medHead);
std::vector<std::string>
generateKeyFindings(const std::vector<DemandPrediction> &predictions,
                    const std::vector<DeptBedAnalysis> &bedAnalyses,
                    const std::vector<BedReallocationSuggestion> &suggestions);

DataAnalysisReport runFullAnalysis(Hospitalization *hosHead, bedInfo *bedHead,
                                   Registration *regHead, Medicine *medHead);

// ===================== 高级分析 =====================

std::vector<std::string>
detectAnomalies(const std::vector<MonthlyDeptStats> &monthlyStats,
