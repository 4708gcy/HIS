...
void savePharmacistData(Pharmacist *pharmacistHead, int count); // 保存到 MySQL（事务）
void savePatientData(Patient *patientHead, int count);          // 保存到 MySQL（事务）

// =====================医疗记录数据保存函数=====================
void saveRegistrations(Registration *regHead, int count);            // 保存到 MySQL（事务）
void saveConsultations(Consultation *conHead, int count);            // 保存到 MySQL（事务）
void saveExaminations(Examination *examHead, int count);             // 保存到 MySQL（事务）
void saveHospitalizations(Hospitalization *hosHead, int count);      // 保存到 MySQL（事务）
void saveMedicationRecords(MedicationRecord *medRecHead, int count); // 保存到 MySQL（事务）
