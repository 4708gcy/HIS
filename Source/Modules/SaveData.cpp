  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO admins (admin_id, username, stored_hash, salt, "
            "login_attempts, is_active, "
            "create_time, gender, age, telephone, email, is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            "username=VALUES(username), stored_hash=VALUES(stored_hash), "
            "salt=VALUES(salt), "
            "login_attempts=VALUES(login_attempts), "
            "is_active=VALUES(is_active), "
            "create_time=VALUES(create_time), gender=VALUES(gender), "
            "age=VALUES(age), "
            "telephone=VALUES(telephone), email=VALUES(email), "
            "is_deleted=VALUES(is_deleted)",
            {current->getUserID(), current->getUsername(),
             current->getStoredHash(), current->getSalt(),
             std::to_string(current->getLoginAttempts()),
             current->getIsAccountActive() ? "1" : "0",
             dbDateTime(current->getCreateTime()), current->getGender(),
             std::to_string(current->getAge()), current->getTelephone(),
             current->getEmail(), current->getIsDeleted() ? "1" : "0"});
      }
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO doctors (doctor_id, username, stored_hash, salt, "
            "login_attempts, is_active, "
            "create_time, gender, age, telephone, email, department, title, "
            "specialty, schedule_info, "
            "consultation_count, examination_count, "
            "hospitalization_apply_count, is_on_duty, is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
            "?) "
            "ON DUPLICATE KEY UPDATE "
            "username=VALUES(username), stored_hash=VALUES(stored_hash), "
            ...
            "examination_count=VALUES(examination_count), "
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO nurses (nurse_id, username, stored_hash, salt, "
            "login_attempts, is_active, "
            "create_time, gender, age, telephone, email, department, level, "
            "schedule_info, "
            ...
            "patient_care_count=VALUES(patient_care_count), "
            "bed_manage_count=VALUES(bed_manage_count), "
            "is_on_duty=VALUES(is_on_duty), is_deleted=VALUES(is_deleted)",
            {current->getUserID(), current->getUsername(),
             current->getStoredHash(), current->getSalt(),
             std::to_string(current->getLoginAttempts()),
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO pharmacists (pharmacist_id, username, stored_hash, "
            "salt, login_attempts, is_active, "
            "create_time, gender, age, telephone, email, department, level, "
            "schedule_info, "
            ...
            "dispense_count=VALUES(dispense_count), "
            ...
            {current->getUserID(), current->getUsername(),
             ...
             current->getEmail(), dbStr(current->department),
             std::to_string(static_cast<int>(current->level)),
             dbStr(current->scheduleInfo), std::to_string(current->reviewCount),
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO patients (patient_id, username, stored_hash, salt, "
            "login_attempts, is_active, "
            "create_time, gender, age, telephone, email, department, address, "
            "id_card_number, "
            "emergency_contact_name, emergency_contact_phone, allergy_history, "
            "past_medical_history, "
            "marital_status, registration_count, consultation_count, "
            ...
            "consultation_count=VALUES(consultation_count), "
            "hospitalization_count=VALUES(hospitalization_count), "
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO registrations (registration_id, patient_id, "
            ...
            "fee_cents=VALUES(fee_cents), status=VALUES(status), "
            "note=VALUES(note), "
            "is_deleted=VALUES(is_deleted)",
            {current->registrationID, current->patientID,
             dbStr(current->department), current->doctorID,
             dbDateTime(current->registerTime), std::to_string(current->fee),
             std::to_string(static_cast<int>(current->status)),
             dbStr(current->note), "0"});

        // Sub-table: related_record_ids — delete-then-insert strategy
        db.executePrepared("DELETE FROM related_record_ids WHERE record_type = "
                           "'registration' AND record_id = ?",
                           {current->registrationID});
        for (const auto &rid : current->relatedRegistrationIDs) {
          db.executePrepared(
              "INSERT INTO related_record_ids (record_type, record_id, "
              "related_id) VALUES ('registration', ?, ?)",
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO consultations (consultation_id, registration_id, "
            "patient_id, doctor_id, "
            "consultation_time, department, chief_complaint, "
            "history_present_illness, "
            "past_medical_history, family_history, preliminary_diagnosis, "
            "is_prescription_reviewed, is_hospitalization_recommended, status, "
            "note, is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            ...
            "is_hospitalization_recommended=VALUES(is_hospitalization_"
            "recommended), "
            "status=VALUES(status), note=VALUES(note), "
            "is_deleted=VALUES(is_deleted)",
            {current->consultationID, current->registrationID,
             current->patientID, current->doctorID,
             dbDateTime(current->consultationTime), dbStr(current->department),
             dbStr(current->chiefComplaint),
             dbStr(current->historyOfPresentIllness),
             dbStr(current->pastMedicalHistory), dbStr(current->familyHistory),
        ...
        for (const auto &pres : current->prescriptions) {
          db.executePrepared(
              "INSERT INTO prescription_items (consultation_id, medicine_id, "
              "name, dosage, frequency, duration, note) "
              "VALUES (?, ?, ?, ?, ?, ?, ?)",
              {current->consultationID, pres.medicineID, pres.name, pres.dosage,
               pres.frequency, pres.duration, dbStr(pres.note)});
        }
        ...
        for (const auto &item : current->examinationlist) {
          db.executePrepared("INSERT INTO examination_items (consultation_id, "
                             "item_name) VALUES (?, ?)",
                             {current->consultationID, item});
        }

        // Sub-table: attachment_files — delete-then-insert
        db.executePrepared("DELETE FROM attachment_files WHERE record_type = "
                           "'consultation' AND record_id = ?",
                           {current->consultationID});
        for (const auto &att : current->attachments) {
          db.executePrepared(
              "INSERT INTO attachment_files (record_type, record_id, "
              "file_path) VALUES ('consultation', ?, ?)",
        ...
        for (const auto &rid : current->relatedConsultationIDs) {
          db.executePrepared(
              "INSERT INTO related_record_ids (record_type, record_id, "
              "related_id) VALUES ('consultation', ?, ?)",
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO examinations (examination_id, consultation_id, "
            "patient_id, doctor_id, "
            "department, item_name, order_time, report_time, report_summary, "
            "fee_cents, status, is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            ...
            "fee_cents=VALUES(fee_cents), status=VALUES(status), "
            "is_deleted=VALUES(is_deleted)",
            {current->examinationID, current->consultationID,
             current->patientID, current->doctorID, dbStr(current->department),
             dbStr(current->itemName), dbDateTime(current->orderTime),
             examReportTime, dbStr(current->reportSummary),
             std::to_string(current->fee),
             std::to_string(static_cast<int>(current->status)), "0"},
        ...
        db.executePrepared("DELETE FROM vital_signs WHERE record_type = "
                           "'examination' AND record_id = ?",
                           {current->examinationID});
        db.executePrepared(
            "INSERT INTO vital_signs (record_type, record_id, temperature_c, "
            "systolic_bp, diastolic_bp, "
            "heart_rate, respiratory_rate, spo2, height, weight, bmi, "
            "pain_score, "
            "waist_circumference, blood_sugar, body_fat, uric_acid, "
            "cholesterol) "
            "VALUES ('examination', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
            "?, ?)",
            {current->examinationID, std::to_string(vs.temperatureC),
             std::to_string(vs.systolicBP), std::to_string(vs.diastolicBP),
             std::to_string(vs.heartRate), std::to_string(vs.respiratoryRate),
             std::to_string(vs.spo2), std::to_string(vs.height),
             std::to_string(vs.weight), std::to_string(vs.bmi),
             std::to_string(vs.painScore),
             std::to_string(vs.waistCircumference),
             std::to_string(vs.bloodSugar), std::to_string(vs.bodyFat),
             std::to_string(vs.uricAcid), std::to_string(vs.cholesterol)});
...
void saveHospitalizations(Hospitalization *hosHead, int count) {
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        db.executePrepared("UPDATE hospitalizations SET is_deleted = 1 WHERE "
                           "hospitalization_id = ?",
        ...
        std::string hosAvailableAdmitTime =
            dbDateTime(current->availableAdmitTime);
        db.executePrepared(
            "INSERT INTO hospitalizations (hospitalization_id, "
            "consultation_id, patient_id, doctor_id, "
            "nurse_id, department, ward_type, bed_number, apply_time, "
            "admit_time, discharge_time, "
            "available_admit_time, deposit_cents, total_cost_cents, status, "
            "is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            "consultation_id=VALUES(consultation_id), "
            "patient_id=VALUES(patient_id), "
            "doctor_id=VALUES(doctor_id), nurse_id=VALUES(nurse_id), "
            "department=VALUES(department), "
            "ward_type=VALUES(ward_type), bed_number=VALUES(bed_number), "
            "apply_time=VALUES(apply_time), "
            "admit_time=VALUES(admit_time), "
            "discharge_time=VALUES(discharge_time), "
            "available_admit_time=VALUES(available_admit_time), "
            "deposit_cents=VALUES(deposit_cents), "
            "total_cost_cents=VALUES(total_cost_cents), status=VALUES(status), "
            "is_deleted=VALUES(is_deleted)",
            {current->hospitalizationID, current->consultationID,
             current->patientID, current->doctorID, current->nurseID,
             dbStr(current->department), dbStr(current->wardType),
             current->bedNumber, dbDateTime(current->applyTime), hosAdmitTime,
             hosDischargeTime, hosAvailableAdmitTime,
            ...
            {false, false, false, false, false, false, false, false, false,
             ...
             hosAvailableAdmitTime
                 .empty(), // availableAdmitTime → SQL NULL when empty
             false, false, false, false});

        // Sub-table: related_record_ids — delete-then-insert
        db.executePrepared("DELETE FROM related_record_ids WHERE record_type = "
                           "'hospitalization' AND record_id = ?",
                           {current->hospitalizationID});
        for (const auto &rid : current->relatedHospitalizationIDs) {
          db.executePrepared(
              "INSERT INTO related_record_ids (record_type, record_id, "
              "related_id) VALUES ('hospitalization', ?, ?)",
...
void saveMedicationRecords(MedicationRecord *medRecHead, int count) {
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        db.executePrepared("UPDATE medication_records SET is_deleted = 1 WHERE "
        ...
        db.executePrepared(
            "INSERT INTO medication_records (medication_record_id, "
            "consultation_id, doctor_id, "
            "pharmacist_id, patient_id, department, create_time, "
            "total_cost_cents, "
            "review_status, status, payment_time, dispense_time, note, "
            "is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            "consultation_id=VALUES(consultation_id), "
            ...
            "dispense_time=VALUES(dispense_time), note=VALUES(note), "
            "is_deleted=VALUES(is_deleted)",
            {current->medRecordID, current->consultationID, current->doctorID,
             current->pharmacistID, current->patientID,
             dbStr(current->department), dbDateTime(current->createTime),
             std::to_string(current->totalCost),
             std::to_string(static_cast<int>(current->reviewStatus)),
             std::to_string(static_cast<int>(current->status)), medPaymentTime,
             medDispenseTime, dbStr(current->note), "0"},
            {false, false, false, false, false, false, false, false, false,
             false,
        ...
        for (const auto &line : current->lines) {
          db.executePrepared(
              "INSERT INTO medication_lines (medication_record_id, "
              "medicine_id, medicine_name, "
              "quantity, unit_price_cents, note) VALUES (?, ?, ?, ?, ?, ?)",
              {current->medRecordID, line.medicineID, line.medicineName,
               std::to_string(line.quantity), std::to_string(line.unitPrice),
               dbStr(line.note)});
        }
      }
      current = current->next;
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO medicines (medicine_id, name, specification, "
            "manufacturer, "
            ...
            "generic_name=VALUES(generic_name), status=VALUES(status), "
            "note=VALUES(note), "
            "is_deleted=VALUES(is_deleted)",
            {current->medicineID, current->name, dbStr(current->specification),
             dbStr(current->manufacturer),
             std::to_string(current->purchasePrice),
             std::to_string(current->salePrice), std::to_string(current->stock),
             std::to_string(current->safetyStock), current->productionDate,
             current->expiryDate, dbStr(current->department),
             current->isSpecial ? "1" : "0", dbStr(current->genericName),
             std::to_string(static_cast<int>(current->status)),
             dbStr(current->note), "0"});

        // Sub-table: medicine_aliases — delete-then-insert
        db.executePrepared("DELETE FROM medicine_aliases WHERE medicine_id = ?",
                           {current->medicineID});
        for (const auto &alias : current->aliases) {
          db.executePrepared("INSERT INTO medicine_aliases (medicine_id, "
                             "alias_name) VALUES (?, ?)",
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        std::string bedPatientID =
            (current->patientID == "#" || current->patientID.empty())
                ? ""
                : current->patientID;
        std::string bedNurseID =
            (current->nurseID == "#" || current->nurseID.empty())
                ? ""
                : current->nurseID;
        db.executePrepared(
            "INSERT INTO bed_info (bed_id, status, ward_type, department, "
            "area_number, "
            "ward_number, bed_number, note, patient_id, nurse_id, is_deleted, "
            "use_times, days_occupied) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            "status=VALUES(status), ward_type=VALUES(ward_type), "
            "department=VALUES(department), "
            "area_number=VALUES(area_number), ward_number=VALUES(ward_number), "
            "bed_number=VALUES(bed_number), note=VALUES(note), "
            "patient_id=VALUES(patient_id), "
            "nurse_id=VALUES(nurse_id), is_deleted=VALUES(is_deleted), "
            "use_times=VALUES(use_times), days_occupied=VALUES(days_occupied)",
            {current->bedID, std::to_string(static_cast<int>(current->status)),
             dbStr(current->wardType), dbStr(current->department),
             std::to_string(current->areaNumber),
             std::to_string(current->wardNumber),
             std::to_string(current->bedNumber), dbStr(current->note),
             bedPatientID, bedNurseID, "0", std::to_string(current->useTimes),
        ...
        // Sub-table: vital_signs — delete-then-insert
        const VitalSigns &vs = current->vitalSigns;
        db.executePrepared("DELETE FROM vital_signs WHERE record_type = 'bed' "
                           "AND record_id = ?",
                           {current->bedID});
        db.executePrepared(
            "INSERT INTO vital_signs (record_type, record_id, temperature_c, "
            ...
            "cholesterol) "
            "VALUES ('bed', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            {current->bedID, std::to_string(vs.temperatureC),
             std::to_string(vs.systolicBP), std::to_string(vs.diastolicBP),
             std::to_string(vs.heartRate), std::to_string(vs.respiratoryRate),
             std::to_string(vs.spo2), std::to_string(vs.height),
             std::to_string(vs.weight), std::to_string(vs.bmi),
  ...
  try {
    ...
    while (current) {
      if (current->isDeleted) {
        ...
        db.executePrepared(
            "INSERT INTO nursing_records (record_id, patient_id, nurse_id, "
            "department, "
            "operation_time, operation_type, detail, vital_signs_snapshot, "
            "note, is_deleted) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            "ON DUPLICATE KEY UPDATE "
            ...
            "vital_signs_snapshot=VALUES(vital_signs_snapshot), "
            "note=VALUES(note), "
            "is_deleted=VALUES(is_deleted)",
            {current->recordID, current->patientID, current->nurseID,
             dbStr(current->department), dbDateTime(current->operationTime),
             std::to_string(current->operationType), dbStr(current->detail),
             dbStr(current->vitalSignsSnapshot), dbStr(current->note), "0"});
      }
      current = current->next;
    }
    db.commit();
