...
// 根据医生职称计算挂号费用
int User::calculateRegistrationFee(DoctorTitle title)
{
    switch (title)
    {
    case DoctorTitle::INTERN:
        return 1000;  // 10.00元 = 1000分
    case DoctorTitle::RESIDENT:
        return 2000;  // 20.00元
    case DoctorTitle::ATTENDING:
        return 3000;  // 30.00元
...
// 根据床位类型和住院天数计算住院费用（单位：分）
int User::calculateHospitalizationFee(std::string wardType, int days)
{
    int dailyRate = 0;
    if (wardType == "普通病房")
        dailyRate = 5000;
...
// 根据检查项目名称计算检查费用（单位：分）
int User::calculateExaminationFee(std::string itemName)
{
    if (itemName == "体温测量")      return 500;
    if (itemName == "血压测量")      return 800;
    if (itemName == "心率测量")      return 500;
    if (itemName == "呼吸频率测量")   return 500;
    if (itemName == "脉搏血氧测量")   return 1000;
    if (itemName == "身高测量")      return 500;
    if (itemName == "体重测量")      return 500;
    if (itemName == "BMI计算")       return 500;
    if (itemName == "疼痛评分")      return 200;
    if (itemName == "腰围测量")      return 500;
    if (itemName == "血糖测量")      return 2000;
    if (itemName == "体脂测量")      return 3000;
    if (itemName == "尿酸测定")      return 2500;
    if (itemName == "血脂测定")      return 2500;
    return 500;
}
