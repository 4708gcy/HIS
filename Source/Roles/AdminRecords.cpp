/**
 * @file AdminRecords.cpp
 * @brief 管理员医疗记录管理功能实现
 * @details 包含挂号、看诊、检查、住院、床位、用药记录、药品等7类记录管理
 */
...
bool Admin::viewAllRegistrations(Registration *&reg, const std::string &department)
{
    ...
    return displayChainByDept<Registration>(reg, department, [](Registration *cur) {
        printRegistrationCard(cur);
    });
}
// 查看指定医生的挂号记录
bool Admin::viewRegistrationsByDoctor(Registration *&reg, const std::string &department)
{
    ...
    return displayChainByFilter<Registration>(reg,
        [&](const Registration *r) {
            return deptMatch(r->department, department) && r->doctorID == doctorID;
        },
        [](const Registration *r) {
            printRegistrationCard(r);
        });
}
// 查看指定患者的挂号记录
bool Admin::viewRegistrationsByPatient(Registration *&reg, const std::string &department)
{
    ...
    return displayChainByFilter<Registration>(reg,
        [&](const Registration *r) {
            return deptMatch(r->department, department) && r->patientID == patientID;
        },
        [](const Registration *r) {
            printRegistrationCard(r);
        });
...
bool Admin::viewRegistrationsByStatus(Registration *&reg, const std::string &department)
{
    ...
    return displayChainByFilter<Registration>(reg,
        [&](const Registration *r) {
            return deptMatch(r->department, department) && r->status == filterStatus;
        },
        [](const Registration *r) {
            printRegistrationCard(r);
        });
...
bool Admin::viewRegistrationsByID(Registration *&reg, const std::string &department)
{
    ...
    return displayChainByFilter<Registration>(reg,
        [&](const Registration *r) {
            return deptMatch(r->department, department) && r->registrationID == regID;
        },
        [](const Registration *r) {
            printRegistrationCard(r);
        });
...
bool Admin::addConsultation(Consultation *&con, const std::string &department, Registration *reg, int &conCounter)
{
    ...
    { // 只显示当前科室的挂号记录，并且状态为已支付的记录
        ...
        {
            printRegistrationCard(currentReg);
            std::cout << ", 科室: " << currentReg->department
                      << std::endl;
        }
...
bool Admin::viewAllHospitalizations(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByDept<Hospitalization>(hos, department, [](Hospitalization *cur) {
        printHospitalizationCard(cur);
    });
}
// 根据患者ID查看住院记录
bool Admin::viewHospitalizationsByPatient(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return deptMatch(h->department, department) && h->patientID == patientID;
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
}
...
bool Admin::viewHospitalizationsByDoctor(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return deptMatch(h->department, department) && h->doctorID == doctorID;
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
...
bool Admin::viewHospitalizationByNurse(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return deptMatch(h->department, department) && h->nurseID == nurseID;
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
...
bool Admin::viewHospitalizationByID(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return h->hospitalizationID == hosID && deptMatch(h->department, department);
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
...
bool Admin::viewHospitalizationsByStatus(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return deptMatch(h->department, department) && h->status == filterStatus;
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
...
bool Admin::viewHospitalizationByWardType(Hospitalization *&hos, const std::string &department)
{
    ...
    return displayChainByFilter<Hospitalization>(hos,
        [&](const Hospitalization *h) {
            return deptMatch(h->department, department) && h->wardType == wardTypeStr;
        },
        [](const Hospitalization *h) {
            printHospitalizationCard(h);
        });
            ...
            if (ishave)
            {
                ...
                if (target)
                {
                    while (true)
                    {
                        ...
                        printHospitalizationCard(target);
                        std::cout << std::endl;
...
bool Admin::viewAllBeds(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByDept<bedInfo>(bed, department, [](bedInfo *cur) {
        printBedCard(cur);
    });
}
// 根据病房类型查看床位信息
bool Admin::viewBedsByWardType(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByFilter<bedInfo>(bed,
        [&](const bedInfo *b) {
            return deptMatch(b->department, department) && b->wardType == wardTypeStr;
        },
        [](const bedInfo *b) {
            printBedCard(b);
        });
...
bool Admin::viewBedsByStatus(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByFilter<bedInfo>(bed,
        [&](const bedInfo *b) {
            return deptMatch(b->department, department) && b->status == filterStatus;
        },
        [](const bedInfo *b) {
            printBedCard(b);
        });
...
bool Admin::viewBedByID(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByFilter<bedInfo>(bed,
        [&](const bedInfo *b) {
            return b->bedID == bedID && deptMatch(b->department, department);
        },
        [](const bedInfo *b) {
            printBedCard(b);
        });
...
bool Admin::viewBedsByNurseID(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByFilter<bedInfo>(bed,
        [&](const bedInfo *b) {
            return deptMatch(b->department, department) && b->nurseID == nurseID;
        },
        [](const bedInfo *b) {
            printBedCard(b);
        });
...
bool Admin::viewBedsByPatientID(bedInfo *&bed, const std::string &department)
{
    ...
    return displayChainByFilter<bedInfo>(bed,
        [&](const bedInfo *b) {
            return deptMatch(b->department, department) && b->patientID == patientID;
        },
        [](const bedInfo *b) {
            printBedCard(b);
        });
...
bool Admin::viewAllMedicationRecords(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByDept<MedicationRecord>(medRec, department, [](MedicationRecord *cur) {
        printMedicationRecordCard(cur);
    });
}
// 根据患者ID查看用药记录
bool Admin::viewMedicationRecordsByPatient(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return r->patientID == patientID && deptMatch(r->department, department);
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
}
// 根据医生ID查看用药记录
bool Admin::viewMedicationRecordsByDoctor(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return r->doctorID == doctorID && deptMatch(r->department, department);
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
...
bool Admin::viewMedicationRecordsByPharmacist(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return r->pharmacistID == pharmacistID && deptMatch(r->department, department);
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
...
bool Admin::viewMedicationRecordByID(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return r->medRecordID == medRecordID && deptMatch(r->department, department);
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
...
bool Admin::viewMedicationRecordsByConsultationID(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return r->consultationID == consultationID && deptMatch(r->department, department);
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
...
bool Admin::viewMedicationRecordsByReviewStatus(MedicationRecord *&medRec, const std::string &department)
{
    ...
    return displayChainByFilter<MedicationRecord>(medRec,
        [&](const MedicationRecord *r) {
            return deptMatch(r->department, department) && r->reviewStatus == filterStatus;
        },
        [](const MedicationRecord *r) {
            printMedicationRecordCard(r);
        });
...
bool Admin::viewAllMedicines(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByDept<Medicine>(med, department, [](Medicine *cur) {
        printMedicineCard(cur);
    });
}
// 根据药品ID查看药品信息
bool Admin::viewMedicineByID(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByFilter<Medicine>(med,
        [&](const Medicine *m) {
            return m->medicineID == medicineID && deptMatch(m->department, department);
        },
        [](const Medicine *m) {
            printMedicineCard(m);
        });
...
bool Admin::viewMedicinesByName(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByFilter<Medicine>(med,
        [&](const Medicine *m) {
            return m->name.find(name) != std::string::npos && deptMatch(m->department, department);
        },
        [](const Medicine *m) {
            printMedicineCard(m);
        });
...
bool Admin::viewMedicinesByStatus(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByFilter<Medicine>(med,
        [&](const Medicine *m) {
            return deptMatch(m->department, department) && m->status == filterStatus;
        },
        [](const Medicine *m) {
            printMedicineCard(m);
        });
...
bool Admin::viewMedicinesByManufacturer(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByFilter<Medicine>(med,
        [&](const Medicine *m) {
            return m->manufacturer.find(manufacturer) != std::string::npos && deptMatch(m->department, department);
        },
        [](const Medicine *m) {
            printMedicineCard(m);
        });
...
bool Admin::viewMedicinesBySafetyStock(Medicine *&med, const std::string &department)
{
    ...
    return displayChainByFilter<Medicine>(med,
        [&](const Medicine *m) {
            return deptMatch(m->department, department) && m->stock < m->safetyStock;
        },
        [](const Medicine *m) {
            printMedicineCard(m);
        });
