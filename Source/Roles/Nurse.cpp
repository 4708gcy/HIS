...
// 护士分配床位给住院患者
void Nurse::assignBed(Hospitalization *&target, bedInfo *&bedHead, Patient *patientHead)
{
    ...
    while (current != nullptr)
    {
            ...
            current->status == bedStatus::AVAILABLE)
        {
            ...
            while (p) {
                if (!p->isDeleted && p->patientID == target->patientID) {
...
// 护士办理患者出院
void Nurse::dischargePatient(Hospitalization *&target, bedInfo *&bedHead, Patient *patientHead)
{
    if (target->status != HospitalizationStatus::ADMITTED)
    {
...
{
    std::cout << "当前押金: " << std::fixed << std::setprecision(2) << (target->deposit / 100.0) << std::endl;
    int newDeposit = inputFeeCheck("请输入新的押金金额: ");
    target->deposit = newDeposit;
    std::ostringstream oss;
    oss << std::fixed << std::setprecision(2) << (target->deposit / 100.0);
    printSuccess("押金已更新！当前押金: " + oss.str());
}

// 护士管理住院记录主函数
void Nurse::manageHospitalizations(Hospitalization *&hosHead, Consultation *&conHead, bedInfo *&bedHead, Patient *patientHead, int &idCounter)
{
    while (true)
    {
        ...
        else if (choice == 2)
        {
            ...
            while (true)
            {
                ...
                if (modifyChoice == 0)
                    ...
                    assignBed(target, bedHead, patientHead);
                else if (modifyChoice == 5)
                    transferBed(target, bedHead);
                else if (modifyChoice == 6)
                    dischargePatient(target, bedHead, patientHead);
...
static std::string nursingOperationTypeName(int type)
{
    switch (type)
    {
    case 1: return "生命体征测量";
    case 2: return "静脉输液";
    case 3: return "肌肉注射";
    case 4: return "口服给药";
    case 5: return "换药";
...
void Nurse::manageNursingRecords(NursingRecord *&nurHead, Patient *patientHead, int &idCounter)
{
    while (true)
    {
        clearScreen();
        printMenuBorder();
        printMenuTitle("护理记录管理");
        printMenuItem(1, "查看护理记录");
        printMenuItem(2, "录入护理记录");
        printMenuItem(3, "修改护理记录");
        printMenuItem(4, "删除护理记录");
        printMenuItem(0, "返回上级菜单");
        printMenuBottom();
        ...
        if (choice == 1) // 查看
        {
            clearScreen();
            printMenuBorder();
            printMenuTitle("选择查看方式");
            printMenuItem(1, "查看全部（本科室）");
            printMenuItem(2, "按患者ID筛选");
            printMenuItem(0, "返回");
            printMenuBottom();

            int viewChoice = selectIntCheck(0, 2);
            std::string filterPatientID;
            if (viewChoice == 2) filterPatientID = inputIDCheck("请输入患者ID: ");

            bool found = false;
            NursingRecord *cur = nurHead;
            while (cur)
            {
                if (!cur->isDeleted && cur->department == this->department &&
                    (cur->nurseID == this->nurseID || viewChoice == 1 || viewChoice == 2) &&
                    (filterPatientID.empty() || cur->patientID == filterPatientID))
                {
                    std::cout << "记录ID: " << cur->recordID
                              << ", 患者ID: " << cur->patientID
                              << ", 护士ID: " << cur->nurseID
                              << ", 时间: " << cur->operationTime
                              << ", 类型: " << nursingOperationTypeName(cur->operationType)
                              << ", 详情: " << (cur->detail.empty() || cur->detail == "#" ? "无" : cur->detail)
                              << ", 备注: " << (cur->note.empty() || cur->note == "#" ? "无" : cur->note)
                              << std::endl;
                    found = true;
                }
                cur = cur->next;
            }
            if (!found) std::cout << "未找到护理记录。" << std::endl;
            pause("护士 > 护理记录管理 > 查看记录");
        }
        else if (choice == 2) // 录入
        {
            // 选择患者：列出本科室当前住院患者
            std::cout << "本科室住院患者列表：" << std::endl;
            Patient *p = patientHead;
            bool hasPatient = false;
            while (p)
            {
                if (!p->isDeleted && p->isHospitalized && p->department == this->department)
                {
                    std::cout << "患者ID: " << p->getUserID() << ", 姓名: " << p->getUsername() << std::endl;
                    hasPatient = true;
                }
                p = p->next;
            }
            if (!hasPatient)
            {
                printWarning("本科室暂无住院患者！");
                pause("护士 > 护理记录管理");
                continue;
            }

            NursingRecord *newRec = new NursingRecord();
            newRec->patientID = inputIDCheck("请输入患者ID: ");
            newRec->nurseID = this->nurseID;
            newRec->department = this->department;
            newRec->operationTime = MyTime::getInstance().getTime();

            // 选择操作类型
            clearScreen();
            printMenuBorder();
            printMenuTitle("选择操作类型");
            printMenuItem(1, "生命体征测量");
            printMenuItem(2, "静脉输液");
            printMenuItem(3, "肌肉注射");
            printMenuItem(4, "口服给药");
            printMenuItem(5, "换药");
            printMenuItem(6, "翻身拍背");
            printMenuItem(7, "导尿");
            printMenuItem(8, "其他");
            printMenuBottom();
            newRec->operationType = selectIntCheck(1, 8);

            newRec->detail = inputStringCheck("请输入操作详情");
            if (newRec->detail == "#") newRec->detail = "";

            // 如果是生命体征测量，录入快照
            if (newRec->operationType == 1)
            {
                std::cout << "请录入生命体征（直接回车跳过）：" << std::endl;
                VitalSigns vs;
                std::string input;
                std::cout << "体温(°C): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.temperatureC = std::stod(trim(input)); } catch (...) {} }
                std::cout << "收缩压(mmHg): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.systolicBP = std::stod(trim(input)); } catch (...) {} }
                std::cout << "舒张压(mmHg): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.diastolicBP = std::stod(trim(input)); } catch (...) {} }
                std::cout << "心率(次/分): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.heartRate = std::stod(trim(input)); } catch (...) {} }
                std::cout << "呼吸频率(次/分): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.respiratoryRate = std::stod(trim(input)); } catch (...) {} }
                std::cout << "血氧饱和度(%): "; std::getline(std::cin, input); if (!input.empty()) { try { vs.spo2 = std::stod(trim(input)); } catch (...) {} }

                std::ostringstream oss;
                oss << vs.temperatureC << ";" << vs.systolicBP << ";" << vs.diastolicBP << ";"
                    << vs.heartRate << ";" << vs.respiratoryRate << ";" << vs.spo2 << ";"
                    << vs.height << ";" << vs.weight << ";" << vs.bmi << ";" << vs.painScore << ";"
                    << vs.waistCircumference << ";" << vs.bloodSugar << ";" << vs.bodyFat << ";"
                    << vs.uricAcid << ";" << vs.cholesterol;
                newRec->vitalSignsSnapshot = oss.str();
            }

            newRec->note = inputStringCheck("请输入备注（直接回车跳过）");
            if (newRec->note == "#") newRec->note = "";

            idCounter++;
            newRec->recordID = "nur" + padId(idCounter, 6);

            newRec->next = nurHead;
            if (nurHead) nurHead->prev = newRec;
            nurHead = newRec;

            printSuccess("护理记录录入成功！");
            LogManager::getInstance().logOperation(this->userID, "护士", "CREATE_NURSING_RECORD",
                "录入了护理记录 " + newRec->recordID);
            pause("护士 > 护理记录管理");
        }
        else if (choice == 3) // 修改
        {
            std::string recID = inputRecordIDCheck("请输入要修改的护理记录ID: ", {"nur"});
            NursingRecord *target = nurHead;
            while (target)
            {
                if (!target->isDeleted && target->recordID == recID && target->nurseID == this->nurseID)
                    break;
                target = target->next;
            }
            ...
            if (!target)
            {
                printError("未找到该护理记录（只能修改自己创建的记录）！");
                pause("护士 > 护理记录管理");
                continue;
            }
            ...
            while (true)
            {
                std::cout << "记录ID: " << target->recordID
                          << ", 详情: " << (target->detail.empty() || target->detail == "#" ? "无" : target->detail)
                          << ", 备注: " << (target->note.empty() || target->note == "#" ? "无" : target->note)
                          << std::endl;

                clearScreen();
                printMenuBorder();
                printMenuTitle("修改护理记录");
                printMenuItem(1, "修改操作详情");
                printMenuItem(2, "修改备注");
                printMenuItem(0, "返回");
                printMenuBottom();

                int modChoice = selectIntCheck(0, 2);
                if (modChoice == 1)
                {
                    target->detail = inputStringCheck("请输入新的操作详情");
                    if (target->detail == "#") target->detail = "";
                    printSuccess("操作详情已更新！");
                }
                else if (modChoice == 2)
                {
        ...
        {
            std::string recID = inputRecordIDCheck("请输入要删除的护理记录ID: ", {"nur"});
            NursingRecord *target = nurHead;
            while (target)
            {
                if (!target->isDeleted && target->recordID == recID && target->nurseID == this->nurseID)
            ...
            if (!target)
            {
                printError("未找到该护理记录（只能删除自己创建的记录）！");
                pause("护士 > 护理记录管理");
                continue;
            }
            ...
            clearScreen();
            printMenuBorder();
            printMenuTitle("确认删除护理记录");
            printMenuItem(1, "确认");
            printMenuItem(0, "取消");
            printMenuBottom();

            if (selectIntCheck(0, 1) == 1)
            {
                target->isDeleted = true;
                printSuccess("护理记录已删除！");
                LogManager::getInstance().logOperation(this->userID, "护士", "DELETE_NURSING_RECORD",
                    "删除了护理记录 " + target->recordID);
            }
            pause("护士 > 护理记录管理");
        }
    }
