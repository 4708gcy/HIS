...
// ==================== 处方审核 ====================
void Pharmacist::reviewPrescriptions(MedicationRecord *&medRecHead, Consultation *conHead, Medicine *&medHead, Patient *patientHead, int &idCounter)
{
    ...
    // 过敏检查
    std::vector<std::string> allergyConflicts;
    if (patientHead)
    {
        Patient *p = patientHead;
        while (p)
        {
            if (!p->isDeleted && p->patientID == con->patientID)
                break;
            p = p->next;
        }
        if (p && !p->allergyHistory.empty() && p->allergyHistory != "#")
        {
            printWarning("患者过敏史: " + p->allergyHistory);

            std::vector<std::string> drugNames;
            for (const auto &pres : con->prescriptions)
                drugNames.push_back(pres.name);

            allergyConflicts = DrugSafety::checkAllergyConflict(p->allergyHistory, drugNames);
            if (!allergyConflicts.empty())
            {
                printWarning("警告：以下处方药品与患者过敏史存在冲突：");
    ...
    if (newStatus == MedicationReviewStatus::APPROVED)
    {
        if (!allergyConflicts.empty())
        {
            printError("存在过敏冲突，无法批准该处方！请选择\"拒绝\"并注明原因。");
            return;
        }
    ...
    while (currentMed != nullptr)
    {
        if (!currentMed->isDeleted && currentMed->status != MedicineStatus::EXPIRED && currentMed->status != MedicineStatus::DISCONTINUED)
        {
            std::cout << "药品ID: " << currentMed->medicineID
                      ...
                      << ", 库存: " << currentMed->stock
                      << ", 销售价格: " << std::fixed << std::setprecision(2) << (currentMed->salePrice / 100.0)
                      << std::endl;
            std::cout << std::endl;
        }
...
{
    std::cout << "当前进价: " << std::fixed << std::setprecision(2) << (target->purchasePrice / 100.0) << std::endl;
...
{
    std::cout << "当前售价: " << std::fixed << std::setprecision(2) << (target->salePrice / 100.0) << std::endl;
