...
// 向看诊记录添加处方信息
void Doctor::addConsultationPrescription(Consultation *&target, Medicine *&medHead, Patient *patientHead)
{
    while (true)
    {
        // 显示当前所有处方信息
        ...
        else
        {
            ...
            while (medCurrent != nullptr)
            {
                if (!medCurrent->isDeleted && medCurrent->medicineID == newPres.medicineID && medCurrent->status == MedicineStatus::NORMAL)
                {
                    ...
                    if (patient && !patient->allergyHistory.empty() && patient->allergyHistory != "#")
                    {
                        std::vector<std::string> drugNames;
                        for (const auto &pres : target->prescriptions)
                            drugNames.push_back(pres.name);
                        drugNames.push_back(newPres.name);

                        auto allergyConflicts = DrugSafety::checkAllergyConflict(patient->allergyHistory, drugNames);
                        if (!allergyConflicts.empty())
                        {
                            printWarning("严重警告：以下药品与患者过敏史存在冲突：");
                            for (const auto &c : allergyConflicts)
                                std::cout << "  - " << c << std::endl;
                            std::cout << "请确认是否仍要继续开具？（1-继续，0-取消）" << std::endl;
                            int confirm = selectIntCheck(0, 1);
                            if (confirm == 0)
                            {
                                printWarning("已取消该药品添加。");
                                medFound = true;
                                break;
                            }
                        }
                    ...
                    static auto interactions = DrugSafety::loadInteractions("../Data/DrugData/interactions.txt");
                    if (!interactions.empty())
                    {
                        std::vector<std::string> existingDrugs;
                        for (const auto &pres : target->prescriptions)
                            existingDrugs.push_back(pres.name);

                        auto ddiConflicts = DrugSafety::checkDDI(newPres.name, existingDrugs, interactions);
                        if (!ddiConflicts.empty())
                        {
                            printWarning("警告：该药品与处方中已有药品存在相互作用：");
                            for (const auto &c : ddiConflicts)
                                std::cout << "  - " << newPres.name << " <-> " << c << std::endl;
                            std::cout << "请确认是否仍要继续开具？（1-继续，0-取消）" << std::endl;
                            int confirmDDI = selectIntCheck(0, 1);
                            if (confirmDDI == 0)
                            {
                                printWarning("已取消该药品添加。");
                                medFound = true;
        ...
        else
        {
            ...
            while (medCurrent != nullptr)
            {
                if (!medCurrent->isDeleted && medCurrent->medicineID == newPres.medicineID && medCurrent->status == MedicineStatus::NORMAL)
                {
                    ...
                    // 药物安全检查 — 与 addConsultationPrescription 保持一致
                    Patient *patient = nullptr;
                    if (patientHead)
                    {
                        Patient *p = patientHead;
...
// 从挂号记录创建看诊记录
bool Doctor::createConsultationByRegistration(Registration *&regHead, Consultation *&conHead, Medicine *&medHead, Patient *patientHead, int &idCounter)
{
    ...
    while (currentReg != nullptr)
    {
        if (!currentReg->isDeleted && currentReg->doctorID == this->doctorID && currentReg->registrationID == regID)
        {
            ...
            initConsultationPrescription(newCon, medHead, patientHead);
...
void Doctor::manageConsultations(Consultation *&conHead, Registration *&regHead, Medicine *&medHead, Patient *patientHead, int &idCounter)
{
    while (true)
    {
            ...
            if (ishave)
            {
                ...
                if (target)
                {
                    while (true)
                    {
                        ...
                        {
                            addConsultationPrescription(target, medHead, patientHead);
                            pause("医生 > 看诊管理");
                        }
        ...
        {
            createConsultationByRegistration(regHead, conHead, medHead, patientHead, idCounter);
            pause("医生 > 看诊管理");
        }
    }
}
        ...
        if (!current->isDeleted && current->doctorID == this->doctorID)
        {
            ...
            if (current->orderTime.substr(0, 10) >= startTime && current->orderTime.substr(0, 10) <= endTime)
            {
                std::cout << "检查ID: " << current->examinationID
                          ...
                          << ", 检查费用: " << std::fixed << std::setprecision(2) << (current->fee / 100.0)
            ...
            if (ishave)
            {
                ...
                if (target)
                {
                    ...
                    std::cout << "检查ID: " << target->examinationID
                              ...
                              << ", 检查费用: " << std::fixed << std::setprecision(2) << (target->fee / 100.0)
