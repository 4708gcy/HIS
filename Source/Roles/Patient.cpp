...
void Patient::payRegistrationFee(Registration *&target)
{
    ...
    std::cout << "需支付的费用为 " << std::fixed << std::setprecision(2) << (target->fee / 100.0) << " 元。您当前账户余额为 " << std::fixed << std::setprecision(2) << (this->balance / 100.0) << " 元。" << std::endl;
    ...
    if (c == 1)
    {
        ...
        oss << std::fixed << std::setprecision(2) << (this->balance / 100.0);
        printSuccess("支付成功！当前账户余额: " + oss.str() + " 元。");
    }else{
...
void Patient::printMedicationRecord(MedicationRecord *current)
{
    ...
    std::cout << "用药记录ID: " << current->medRecordID << std::endl
              ...
              << "总费用: " << std::fixed << std::setprecision(2) << (current->totalCost / 100.0) << " 元" << std::endl
    ...
    if (!current->lines.empty())
    {
        ...
        for (const auto &line : current->lines)
        {
            std::cout << "    =======================" << std::endl
                      ...
                      << "    单价: " << std::fixed << std::setprecision(2) << (line.unitPrice / 100.0) << " 元" << std::endl
                      << "    总价: " << std::fixed << std::setprecision(2) << (line.quantity * line.unitPrice / 100.0) << " 元" << std::endl
...
void Patient::payMedicationFee(MedicationRecord *&target)
{
    ...
    std::cout << "需支付的费用为 " << std::fixed << std::setprecision(2) << (target->totalCost / 100.0) << " 元。您当前账户余额为 " << std::fixed << std::setprecision(2) << (this->balance / 100.0) << " 元。" << std::endl;
    ...
    if (confirm == 1)
    {
        ...
        std::ostringstream oss;
        oss << std::fixed << std::setprecision(2) << (target->totalCost / 100.0);
        printSuccess("支付成功！扣除费用 " + oss.str() + " 元。");
        oss.str("");
        oss << std::fixed << std::setprecision(2) << (this->balance / 100.0);
        printSuccess("当前账户余额: " + oss.str() + " 元");
...
static void printHospitalizationRecord(Hospitalization *current, User *u)
{
    ...
    std::cout << "住院记录ID: " << current->hospitalizationID << std::endl
              ...
              << "押金: " << std::fixed << std::setprecision(2) << (current->deposit / 100.0) << " 元" << std::endl
              << "总费用: " << std::fixed << std::setprecision(2) << (current->totalCost / 100.0) << " 元" << std::endl
...
void Patient::applyForDischarge(Hospitalization *&hosHead, bedInfo *&bedHead)
{
    ...
    int calculatedCost = calculateHospitalizationFee(current->wardType, days);

    std::cout << "根据您的入出院天数(" << days << "天)，以及病房类型(" << current->wardType << ")，" << std::endl;
    std::cout << "总费用为 " << std::fixed << std::setprecision(2) << (calculatedCost / 100.0) << " 元。您已缴纳了押金 " << std::fixed << std::setprecision(2) << (current->deposit / 100.0) << " 元。" << std::endl;

    int diff = calculatedCost - current->deposit;
    if (diff > 0)
    {
        std::cout << "您还需补交欠费 " << std::fixed << std::setprecision(2) << (diff / 100.0) << " 元。您的账户余额为 " << std::fixed << std::setprecision(2) << (this->balance / 100.0) << " 元。" << std::endl;
        ...
        if (confirm == 1)
        {
            ...
            std::ostringstream oss;
            oss << std::fixed << std::setprecision(2) << (this->balance / 100.0);
            printSuccess("扣款成功。新余额：" + oss.str() + " 元。");
        }
        else
        {
    ...
    else if (diff <= 0)
    {
        ...
        this->balance += refund;
        std::ostringstream oss;
        oss << std::fixed << std::setprecision(2) << (refund / 100.0);
        printSuccess("您的押金已足够支付，系统将为您退还余额 " + oss.str() + " 元。");
        oss.str("");
        oss << std::fixed << std::setprecision(2) << (this->balance / 100.0);
        printSuccess("退款成功。新余额：" + oss.str() + " 元。");
    }
...
void Patient::payHospitalizationDeposit(Hospitalization *&hosHead)
{
    ...
    int requiredDeposit = 100000; // 默认收取 1000 元作为基础押金（单位：分）
    if (current->wardType == "ICU病房" || current->wardType == "VIP病房")
    {
        requiredDeposit = 500000;
    }
    std::cout << "根据该记录(" << current->wardType << ")，您需要先缴纳床位押金: " << std::fixed << std::setprecision(2) << (requiredDeposit / 100.0) << " 元。" << std::endl;
    std::cout << "您当前的余额为 " << std::fixed << std::setprecision(2) << (this->balance / 100.0) << " 元。" << std::endl;
    ...
    if (confirm == 1)
    {
        ...
        std::ostringstream oss;
        oss << std::fixed << std::setprecision(2) << (this->balance / 100.0);
        printSuccess("缴纳成功！余额 " + oss.str() + " 元。待护士为您排床处理。");
    }
    else
    {
        printWarning("已取消缴纳。");
    ...
    while (true)
    {
        ...
        else if (choice == 1)
        {
            printInfoCard("患者个人信息", {
                {"患者ID", patientID}, {"姓名", username}, {"性别", gender}, {"年龄", std::to_string(age)}, {"身份证号", idCardNumber}, {"联系电话", telephone}, {"邮箱地址", email}, {"家庭住址", address}, {"紧急联系人姓名", emergencyContactName}, {"紧急联系人电话", emergencyContactPhone}, {"过敏史", allergyHistory}, {"既往病史", pastMedicalHistory}, {"婚姻状况", patientMaritalStatusToString(maritalStatus)}, {"账户余额", [this](){ std::ostringstream o; o << std::fixed << std::setprecision(2) << (this->balance / 100.0); return o.str(); }() + std::string(" 元")}, {"是否住院中", isHospitalized ? "是" : "否"}, {"累计挂号次数", std::to_string(registrationCount)}, {"累计就诊次数", std::to_string(consultationCount)}, {"累计住院次数", std::to_string(hospitalizationCount)}, {"累计用药次数", std::to_string(medicationCount)}, {"账户创建时间", createTime}
            });
            pause("患者 > 个人信息管理");
        }
        else if (choice == 2)
        ...
        {
            std::cout << "当前的账户余额: " << std::fixed << std::setprecision(2) << (this->balance / 100.0) << " 元" << std::endl;
            int amount = inputFeeCheck("请输入充值金额 (输入0取消充值): ");
            if (amount > 0)
            {
                this->balance += amount;
                std::ostringstream oss;
                ...
                std::string balStr;
                { std::ostringstream o; o << std::fixed << std::setprecision(2) << (this->balance / 100.0); balStr = o.str(); }
                printSuccess("成功充值 " + oss.str() + " 元。当前余额: " + balStr + " 元");
            }
            else
            {
                printWarning("已取消充值。");
